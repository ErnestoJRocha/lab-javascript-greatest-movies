...
function getAllDirectors(arrayOfMovies) {
  let directors = arrayOfMovies.map(function(element){
...
return Math.round((scores / arr.length)*100)/100;
}
...
function orderAlphabetically(array){
  const alfabeticallOrder = array.map(function(movie){
    return movie.title;
  }).sort(function(a,b){
    return a.localeCompare(b);
  }).slice(0,20);
  
  return alfabeticallOrder;
}
